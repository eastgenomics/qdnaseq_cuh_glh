...
mydir = "~/Documents/Projects/cancer/sWGS/"
subfolder = "case5_down_500bin"
case5_down_500bin = read.csv(paste(mydir,"/", subfolder, "/", sample ,"_TP_FP_TN.tsv", sep =""), sep="\t", header = T)
rownames(case5_down_500bin) <- paste(rownames(case5_down_500bin), subfolder, sep = "_")

subfolder = "case5_down_100bin"
case5_down_100bin = read.csv(paste(mydir,"/", subfolder, "/", sample ,"_TP_FP_TN.tsv", sep =""), sep="\t", header = T)
rownames(case5_down_100bin) <- paste(rownames(case5_down_100bin), subfolder, sep = "_")

subfolder = "case5_down_50bin"
case5_down_50bin = read.csv(paste(mydir,"/", subfolder, "/", sample ,"_TP_FP_TN.tsv", sep =""), sep="\t", header = T)
rownames(case5_down_50bin) <- paste(rownames(case5_down_50bin), subfolder, sep = "_")

subfolder = "case5_down_15bin"
case5_down_15bin = read.csv(paste(mydir,"/", subfolder, "/", sample ,"_TP_FP_TN.tsv", sep =""), sep="\t", header = T)
rownames(case5_down_15bin) <- paste(rownames(case5_down_15bin), subfolder, sep = "_")

subfolder = "case5_down_5bin"
case5_down_5bin = read.csv(paste(mydir,"/", subfolder, "/", sample ,"_TP_FP_TN.tsv", sep =""), sep="\t", header = T)
rownames(case5_down_5bin) <- paste(rownames(case5_down_5bin), subfolder, sep = "_")

case5 <- rbind(case5_down_5bin, case5_down_15bin, case5_down_50bin, case5_down_100bin, case5_down_500bin)
case5$info <- rownames(case5)
case5 <- separate(data = case5, col = info, sep = "_", into = c("SWGS", "sample", "sample_tissue",
                                                     "tissue_type", "case", "downsampled", "bin"))
case5$tissue_type <- gsub('\\s+', '', case5$tissue_type)
case5$bin <- as.factor(gsub('bin*.', '', case5$bin))
case5$bin <- factor(gsub('bin*.', '', case5$bin), levels=c(5,10,15,50,100,500))

case5 <- separate(data = case5, col = sample_tissue, sep = 4, into = c("ID", "repeats"))
#replace null with second
case5$repeats <- gsub(' ', 'O', case5$repeats)

# melt the data to merge everything
case5_melt <- melt(case5[,c("TP", "FP", "TN", "repeats", "tissue_type", "bin")], id.vars=c("tissue_type", "bin", "repeats"))

case5_FF <- case5[which(case5$tissue_type %in% "FF"),]
case5_FFPE <- case5[which(case5$tissue_type %in% "FFPE"),]


# ggplot(case5_melt, aes(x = bin, y = value, fill = variable)) +
#   geom_col(stat="identity", width=.5, position = "dodge") +
#   geom_line(stat="identity", aes(group = bin, fill = variable))
# 
# ggplot(case5_FF, aes(x = bin, y = TP, group = case5_FF$repeats)) +
#   geom_line()
# 
# ggplot(case5_FF,aes(x = bin, y = TP, fill = case5_FF$repeats)) +
