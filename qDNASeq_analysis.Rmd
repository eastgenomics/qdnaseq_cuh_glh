...
bed_intersect <- function(sample_bed, truth_bed){
  ...
  # intersect
  pred.int.truth <- bedr.join.region(sample_bed.sort, truth_bed.sort, check.chr = F, verbose = F)
  # print
...
for(i in 1:length(C5_samples)) { 
  ...
  Case_truthvspredicted_df[i,1] <- nrow(bed_intersect(sample_bed = get(paste(N, "_bed", sep="")), truth_bed = truth_bed))
  # row = sample, column = FP
  Case_truthvspredicted_df[i,2] 
