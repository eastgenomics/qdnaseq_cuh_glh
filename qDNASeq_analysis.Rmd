...
bed_intersect_no_overlap <- function(sample_bed, truth_bed){
  # both dataframes must be chr,start,end
  # truth is expected to have X or Y chr 
  # pred is not expected to have X or Y 
...
```{r}
bed_func <- function(sample_bed, truth_bed){
  # clean data
  truth_bed$start <- as.integer(truth_bed$start)
  truth_bed$end <- as.integer(truth_bed$end)
  sample_bed$chr <- as.character(sample_bed$chr)
  ...
  # function
  pred.int.truth <- bedr.join.region(
                      sample_bed.sort, truth_bed.sort,
                      check.chr = F, verbose = F)
  return(pred.int.truth)
  
}

nrow(bed_func(sample_bed = SWGS_N6_4627_bed, truth_bed = Case_WGS_location_bed))
```
```{r biomart_genes}
# find the genes in the TP regions
tmp <- bed_intersect(sample_bed = SWGS_N6_4627_bed, truth_bed = Case_WGS_location_bed)
