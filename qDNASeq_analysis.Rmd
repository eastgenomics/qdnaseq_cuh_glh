...
# Original 50kbp:
subfolder = ""
case_norm = read.csv(paste(mydir,"/", subfolder, "/", sample ,"_TP_FP_TN.tsv", sep =""), sep="\t", header = T)
rownames(case_norm) <- paste(rownames(case_norm), subfolder, sep = "_")

# Merged 50kbp:
subfolder = paste(case, "_merged_50bin", sep = "")
case_merged = read.csv(paste(mydir,"/", subfolder, "/", sample ,"_TP_FP_TN.tsv", sep =""), sep="\t", header = T)
rownames(case_merged) <- paste("SWGS", gsub('_merged', '' , rownames(case_merged)), subfolder, sep = "_")


# Downsampled:
subfolder= paste(case, "_down_500bin", sep = "")
case_down_500bin = read.csv(paste(mydir,"/", subfolder, "/", sample ,"_TP_FP_TN.tsv", sep =""), sep="\t", header = T)
rownames(case_down_500bin) <- paste(rownames(case_down_500bin), subfolder, sep = "_")
subfolder = paste(case, "_down_100bin", sep = "") 
case_down_100bin = read.csv(paste(mydir,"/", subfolder, "/", sample ,"_TP_FP_TN.tsv", sep =""), sep="\t", header = T)
rownames(case_down_100bin) <- paste(rownames(case_down_100bin), subfolder, sep = "_")
subfolder = paste(case, "_down_50bin", sep = "") 
case_down_50bin = read.csv(paste(mydir,"/", subfolder, "/", sample ,"_TP_FP_TN.tsv", sep =""), sep="\t", header = T)
rownames(case_down_50bin) <- paste(rownames(case_down_50bin), subfolder, sep = "_")
subfolder = paste(case, "_down_15bin", sep = "") 
case_down_15bin = read.csv(paste(mydir,"/", subfolder, "/", sample ,"_TP_FP_TN.tsv", sep =""), sep="\t", header = T)
rownames(case_down_15bin) <- paste(rownames(case_down_15bin), subfolder, sep = "_")
subfolder = paste(case, "_down_5bin", sep = "") 
case_down_5bin = read.csv(paste(mydir,"/", subfolder, "/", sample ,"_TP_FP_TN.tsv", sep =""), sep="\t", header = T)
rownames(case_down_5bin) <- paste(rownames(case_down_5bin), subfolder, sep = "_")


## join all data and modify it 
case_all <- rbind(case_norm, case_merged, case_down_5bin, case_down_15bin, case_down_50bin, case_down_100bin, case_down_500bin)
# get total CN
case_all$CN <- rowSums(case_all) 

# Make the TP,FP and TN into percentages
case_all[,c("TP_percent", "FP_percent", "TN_percent")] <- NA
for (i in 1:nrow(case_all)){
  case_all[i,"TP_percent"] <- (case_all[i,"TP"]/case_all[i,"CN"]) * 100
  case_all[i,"FP_percent"] <- (case_all[i,"FP"]/case_all[i,"CN"]) * 100
  case_all[i,"TN_percent"] <- (case_all[i,"TN"]/case_all[i,"CN"]) * 100
}
...
case_all$downsampled[is.na(case_all$downsampled)] <- "normal"
...
case_melt <- melt(case_all[,c("TP_percent", "FP_percent", "TN_percent", "repeats", "tissue_type", "bin")], id.vars=c("tissue_type", "bin", "repeats"))
# downsamples data only
case_all_down <- case_all[which(case_all$downsampled %in% "down"),]
case_all_down_melt <- melt(case_all_down[,c("TP_percent", "FP_percent", "TN_percent", "repeats", "tissue_type", "bin")], id.vars=c("tissue_type", "bin", "repeats"))
case_melt2 <- melt(case_all[,c("TP_percent", "repeats", "tissue_type", "bin", "downsampled")], id.vars=c("tissue_type", "bin", "repeats", "downsampled"))
...
ggplot(case_all_down_melt[which(case_all_down_melt$tissue_type %in% "FF"),], aes(x = bin, y = value, fill = variable, linetype = repeats, color = repeats)) +
  ...
  labs(title = "Percentages of TP, FP and FN of downsampled case 5 sample across different bins for FF tissue",
       y = "Percentage", x = "bins")
ggplot(case_all_down_melt[which(case_all_down_melt$tissue_type %in% "FFPE"),], aes(x = bin, y = value, fill = variable, linetype = repeats, color = repeats)) +
  ...
  labs(title = "Percentages of TP, FP and FN of downsampled case 5 sample across different bins for FF tissue",
       y = "Percentage", x = "bins")
ggplot(case_melt2[which(case_melt2$tissue_type %in% "FFPE"),], aes(x = bin, y = value, fill = downsampled, linetype = repeats, color = repeats)) +
  geom_col(stat="identity", width=.5, position = "dodge") + 
  labs(title = "Percentages of TP normal, downsampled and merged case 5 sample across different bins for FFPE tissue",
       y = "Percentage", x = "bins")
ggplot(case_melt2[which(case_melt2$tissue_type %in% "FF"),], aes(x = bin, y = value, fill = downsampled, linetype = repeats, color = repeats)) +
  geom_col(stat="identity", width=.5, position = "dodge") + 
  labs(title = "Percentages of TP normal, downsampled and merged case 5 sample across different bins for FF tissue",
       y = "Percentage", z = "bins")

###### Plot to PDF
pdf(file = paste(mydir,sample, "_sWGSvsWGS.pdf", sep = ""),
    width = 10,  height = 4)
ggplot(case_all_down_melt[which(case_all_down_melt$tissue_type %in% "FF"),], aes(x = bin, y = value, fill = variable, linetype = repeats, color = repeats)) +
  geom_col(stat="identity", width=.5, position = "dodge") + 
  labs(title = "Percentages of TP, FP and FN of downsampled case 5 sample across different bins for FF tissue",
