...
make_bed_from_sum_df <- function(SWGS_sum) {
  ...
  SWGS_sum <- as.data.frame(lapply(SWGS_sum, as.integer))
  return(SWGS_sum)
...
#### How many CN in our samples and truth dataset
...
truth_bed <- Case_WGS_location[,c("chr", "start", "end")]
truth_bed <- as.data.frame(lapply(truth_bed, as.integer))

# save the three columns
write.table(truth_bed, 
...
C5_samples<- unique(metadata[which(metadata$CUH_number %in% sample), "samplename"])
...
truth_bed$chr <- paste("chr", truth_bed$chr, sep = "")
SWGS_N6_4627_bed$chr <- paste("chr", SWGS_N6_4627_bed$chr, sep = "")
# intersect each sample against truth
if (check.binary("bedtools")) {
  
    # sort data
    truth_bed.sort <- bedr.sort.region(truth_bed)
    SWGS_N6_4627_bed.sort <- bedr.sort.region(SWGS_N6_4627_bed)

    # intersect / join
    a.int1 <- bedr.join.region(SWGS_N6_4627_bed.sort, truth_bed.sort);
